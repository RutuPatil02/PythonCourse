...
def word_score(word):
    ...
    >>> word_score('DR')
    0
    >>> word_score('DRUDGERYAW')
    30
    >>> word_score('DRawersASWCFRAWE')
    48
    """
...
def update_score(player_info, word):
    ...
    ['Jonathan', 7]
    >>> update_score(['Jonathan', 7], 'ANTWERA')
    ['Jonathan', 21]
    """
...
def num_words_on_board(board, words):
    ...
    3
    >>> num_words_on_board([['A', 'N', 'T', 'T'], ['X', 'S', 'O', 'B']], ['ANT', 'BOX', 'NS','SOB', 'TO'])
    4
    """
    new_board = []

    for i in range(len(board)):
        new_board.append(make_str_from_row(board, i))

    for j in range(len(board[0])):
        new_board.append(make_str_from_column(board, j))
